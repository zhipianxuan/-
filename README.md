...
1. [pytorch text：Torch文本相关的内容](https://github.com/pytorch/text): Torch text related contents.  
2. [pytorch-seq2seq：PyTorch实现序列到序列（seq2seq）模型的框架。](https://github.com/IBM/pytorch-seq2seq): A framework for sequence-to-sequence (seq2seq) models implemented in PyTorch.  
3. [anuvada：PyTorch实现的 NLP 的可解释模型](https://github.com/Sandeep42/anuvada): Interpretable Models for NLP using PyTorch.
4. [audio：PyTorch的简单音频 I/O](https://github.com/pytorch/audio): simple audio I/O for pytorch.
5. [loop：用于生成多个说话人语音的方法](https://github.com/facebookresearch/loop): A method to generate speech across multiple speakers
6. [fairseq-py：用 Python 写的 Facebook AI 研究院提出的序列到序列工具集](https://github.com/facebookresearch/fairseq-py): Facebook AI Research Sequence-to-Sequence Toolkit written in Python.
7. [speech：PyTorch ASR 实现](https://github.com/awni/speech): PyTorch ASR Implementation.
8. [OpenNMT-py：开源 PyTorch 神经机器翻译](https://github.com/OpenNMT/OpenNMT-py): Open-Source Neural Machine Translation in PyTorch http://opennmt.net 
9. [neuralcoref：基于神经网络和 spaCy 的当前最佳共指消解](https://github.com/huggingface/neuralcoref): State-of-the-art coreference resolution based on neural nets and spaCy huggingface.co/coref
10. [sentiment-discovery：用于鲁棒情感分类的大规模无监督语言建模](https://github.com/NVIDIA/sentiment-discovery): Unsupervised Language Modeling at scale for robust sentiment classification.
11. [MUSE：用于多语言无监督/监督词向量的库](https://github.com/facebookresearch/MUSE): A library for Multilingual Unsupervised or Supervised word Embeddings
12. [nmtpytorch：PyTorch 神经机器翻译框架](https://github.com/lium-lst/nmtpytorch): Neural Machine Translation Framework in PyTorch.
13. [pytorch-wavenet：快速语音生成 WaveNet 的实现](https://github.com/vincentherrmann/pytorch-wavenet): An implementation of WaveNet with fast generation
14. [Tacotron-pytorch：端到端语音合成的 PyTorch 实现](https://github.com/soobinseo/Tacotron-pytorch): Tacotron: Towards End-to-End Speech Synthesis.
15. [AllenNLP：开源 NLP 研究库，基于 PyTorch](https://github.com/allenai/allennlp): An open-source NLP research library, built on PyTorch.
16. [PyTorch-NLP：yTorch NLP 库](https://github.com/PetrochukM/PyTorch-NLP): Text utilities and datasets for PyTorch pytorchnlp.readthedocs.io
17. [quick-nlp：基于 FastAI 的 PyTorch NLP 库](https://github.com/outcastofmusic/quick-nlp): Pytorch NLP library based on FastAI. 
18. [TTS：Text2Speech 的深度学习实现](https://github.com/mozilla/TTS): Deep learning for Text2Speech
19. [LASER：语言无关的句子表征的 PyTorch 实现](https://github.com/facebookresearch/LASER): Language-Agnostic SEntence Representations
20. [pyannote-audio：说话人追踪的神经构建块：语音激活检测、说话人变化检测、说话人嵌入](https://github.com/pyannote/pyannote-audio): Neural building blocks for speaker diarization: speech activity detection, speaker change detection, speaker embedding
...
1. [pytorch vision：计算机视觉领域的数据集、转换和模型](https://github.com/pytorch/vision): Datasets, Transforms and Models specific to Computer Vision.
2. [pt-styletransfer：PyTorch 实现的神经分割迁移，作为一个类](https://github.com/tymokvo/pt-styletransfer): Neural style transfer as a class in PyTorch.
3. [OpenFacePytorch：使用 OpenFace's nn4.small2.v1.t7 模型的 PyTorch 模块](https://github.com/thnkim/OpenFacePytorch):  PyTorch module to use OpenFace's nn4.small2.v1.t7 model
4. [img_classification_pk_pytorch：快速对比你的图像分类模型和当前最佳模型（例如 DenseNet、ResNet，等）](https://github.com/felixgwu/img_classification_pk_pytorch): Quickly comparing your image classification models with the state-of-the-art models (such as DenseNet, ResNet, ...)
5. [SparseConvNet：子流形稀疏卷积网络](https://github.com/facebookresearch/SparseConvNet): Submanifold sparse convolutional networks.
6. [Convolution_LSTM_pytorch：多层卷积 LSTM 模块](https://github.com/automan000/Convolution_LSTM_pytorch): A multi-layer convolution LSTM module
7. [face-alignment：基于 PyTorch 的 2D 和 3D 人脸对齐库](https://github.com/1adrianb/face-alignment): :fire: 2D and 3D Face alignment library build using pytorch adrianbulat.com
8. [pytorch-semantic-segmentation：语义分割的 PyTorch 实现](https://github.com/ZijunDeng/pytorch-semantic-segmentation): PyTorch for Semantic Segmentation.
9. [RoIAlign.pytorch：这是一个 PyTorch 版本的 RoIAlign。这个实现基于 crop_and_resize，并支持 CPU 和 GPU 上的前向传播、反向传播](https://github.com/longcw/RoIAlign.pytorch): This is a PyTorch version of RoIAlign. This implementation is based on crop_and_resize and supports both forward and backward on CPU and GPU.
10. [pytorch-cnn-finetune：PyTorch 实现的微调预训练卷积神经网络](https://github.com/creafz/pytorch-cnn-finetune): Fine-tune pretrained Convolutional Neural Networks with PyTorch.
11. [detectorch：PyTorch 实现的 detectron 图像分割模型](https://github.com/ignacio-rocco/detectorch): Detectorch - detectron for PyTorch
12. [Augmentor：Python 图像增强库](https://github.com/mdbloice/Augmentor): Image augmentation library in Python for machine learning. http://augmentor.readthedocs.io
13. [s2cnn：这个库包含一个用于球面信号（例如，全向摄像机、地球信号等）的 SO(3) 等变 CNN 的 PyTorch 实现](https://github.com/jonas-koehler/s2cnn): 
...
1. [ptstat：PyTorch 上的概率编程和统计推理](https://github.com/stepelu/ptstat): Probabilistic Programming and Statistical Inference in PyTorch
2. [pyro：基于 PyTorch 和 Python 的深度全局概率编程](https://github.com/uber/pyro): Deep universal probabilistic programming with Python and PyTorch http://pyro.ai
3. [probtorch：Probabilistic Torch 是深度生成模型的 PyTorch 扩展库](https://github.com/probtorch/probtorch): Probabilistic Torch is library for deep generative models that extends PyTorch.
4. [paysage：Python/PyTorch 中的无监督学习和生成模型](https://github.com/drckf/paysage): Unsupervised learning and generative models in python/pytorch.
5. [pyvarinf：Python 包，方便在 PyTorch 中利用变分推理执行贝叶斯深度学习方法。](https://github.com/ctallec/pyvarinf): Python package facilitating the use of Bayesian Deep Learning methods with Variational Inference for PyTorch. 
6. [pyprob：一个基于 PyTorch 的概率编程和推理编译库。](https://github.com/probprog/pyprob): A PyTorch-based library for probabilistic programming and inference compilation.
7. [mia：对机器学习模型运行成员推理攻击的库](https://github.com/spring-epfl/mia): A library for running membership inference attacks against ML models. 
